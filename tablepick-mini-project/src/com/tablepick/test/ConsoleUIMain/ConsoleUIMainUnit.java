...
public class ConsoleUIMainUnit {
	private static ConsoleUIMainUnit instance;

	private AccountDao accountdao;
	private TablePickSerivceCommon tablePickServiceCommon;

	// 싱글톤 인스턴스 static으로 선언
	public static ConsoleUIMainUnit getInstance() {
		if (instance == null) {
			synchronized (ConsoleUIMainUnit.class) {
				if (instance == null) {
					instance = new ConsoleUIMainUnit();
				}
			}
		}
		return instance;
	...
	public void execute() {
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
			exit: while (true) {
				...
				switch (inputoption) {
				case "1" -> loginView(reader);
				case "2" -> addAccountView(reader);
				case "3" -> {
	...
	private void loginView(BufferedReader reader) {
		try {
	...
	private void addAccountView(BufferedReader reader) {
		try {
